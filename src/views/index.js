...
export default class StartIndex extends Component {
    ...
    componentDidMount() {
        ...
        Promise.all([wallets]).then(res => {
            ...
            if (!wallet) return new Nav(this).empty('Start');
            if (!wallet.build) return new Nav(this).empty('Start');
            //判断版本缓存
            ...
            if (Tools.versionContrast(conf.AppVersion, build, wallet.version, wallet.build))
                return new Nav(this).empty('VersionUpdate');
            return new Nav(this).empty('Main');
        });
    }
